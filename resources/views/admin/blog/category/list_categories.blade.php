        ...
        <div class="box-body with-border">
            <div class="row">
                ...
                <div class="col-md-2 text-right mr-3">
                    <a href="{{ route('admin.blog_category.new') }}"> <i class="fa fa-plus"></i> Yeni Kategori Ekle</a>&nbsp;
                    <a href="{{ route('admin.blog_category') }}"><i class="fa fa-refresh"></i>&nbsp;Yenile</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-xs-12">
            <div class="box ">
                <div class="box-header">
                    ...
                    <div class="box-tools">
                        <form action="{{ route('admin.blog_category') }}" method="get" id="form">
                            <div class="row">
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        @if(count($list) > 0)
                            @foreach($list as $l)
                                <tr>
                                    <td>{{ $l ->id }}</td>
                                    <td><a href="{{ route('admin.blog_category.edit',$l->id) }}"> {{ $l->title }}</a></td>
                                    <td>{{ $l -> parent_cat->title}}</td>
                                    <td>{{ $l -> row}}</td>
                                    <td>{{ $l ->slug }}</td>
                                    <td><i class="fa fa-{{ $l -> active == false ? 'times text-red' : 'check text-green' }}"></i></td>
                                    <td><a href="{{ route('admin.blog_category.delete',$l->id) }}" onclick="return confirm('Silmek istediğine emin misin ?')"><i
